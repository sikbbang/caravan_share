...
@router.post("/caravans", response_model=Caravan, dependencies=[Depends(get_current_user)])
...
@router.get("/caravans", response_model=List[Caravan], dependencies=[Depends(get_current_user)])
...
@router.delete("/caravans/{caravan_id}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(get_current_user)])
